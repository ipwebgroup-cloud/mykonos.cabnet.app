...
- keep the Inquiry Queue scan-friendly by using lighter repeat-guest summary rendering on each row and simple 5 / 10 / 50 / All row controls when operators only need a smaller visible slice of the list, while keeping those controls visually aligned in a clean second toolbar row that does not fight the native search/setup controls
