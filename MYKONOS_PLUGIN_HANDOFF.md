...
- keep default visible columns focused on the best daily scan fields
...
- `v6.41.74 inquiry queue default visibility polish patch`
- plugin tracking `2.4.23`

This patch stays backend-only and does not touch `/plan`.
It keeps the Inquiry Queue aligned with the lighter default-scan philosophy already applied to Loyalty Continuity by hiding lower-priority default queue columns while keeping them available through List Setup.
...
- default visible columns feel lighter and easier to scan
- denser queue details remain available in List Setup
- Loyalty Continuity still follows the same lighter default-scan posture

---

## Safest next step
If the queue still feels dense after this patch, the next real pass should inspect:
- whether Inquiry Queue helper labels can shorten further without reducing clarity
- whether queue row partials can compress repeated wording
- whether Inquiry Queue and Loyalty Continuity now need one shared list-setup philosophy note
- whether any remaining default-visible list columns should move behind List Setup
