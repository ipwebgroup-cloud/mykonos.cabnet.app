...
It adds a compact contact-quality strip directly to the inquiry record so operators can compare reply-channel posture and contact-quality posture from one backend screen.
...
- a new Contact Quality strip appears on the inquiry record
...
- improves contact-quality judgment directly inside the inquiry record
- keeps contact guidance visible without changing workflow state
...
After this backend contact-quality pass, the next strong step should be one of:
- add stronger hospitality tone refinement to the guest confirmation template
- add a compact internal guest-profile strip to the inquiry record
