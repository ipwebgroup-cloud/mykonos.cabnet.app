...
- `v7.08.00 inquiry record response-quality strip`
- plugin tracking `2.4.23`

This patch stays backend-only and does not touch `/plan`, SMTP, schema, or queue logic.
It adds a compact response-quality strip directly to the inquiry record so operators can compare request completeness and response quality posture from one backend screen.
...
- a new Response Quality strip appears on the inquiry record
...
- improves response-quality judgment directly inside the inquiry record
- keeps response guidance visible without changing workflow state
...
After this backend response-quality pass, the next strong step should be one of:
- add stronger hospitality tone refinement to the guest confirmation template
- add a compact internal confidence strip to the inquiry record
